...
class Unit extends JI_Controller
{
	...
	public function __construct()
	{
		...
		$this->load("api_admin/a_unit_model", 'aum');
	}
	...
	public function index()
	{
		...
		$datatable = $this->aum->datatable()->initialize();
		$dcount = $this->aum->count($b_user_id, $datatable->keyword(), $is_active, $a_unit_id);
		$ddata = $this->aum->data(
			$b_user_id,
			$datatable->page(),
			$datatable->pagesize(),
		...
		foreach ($ddata as &$gd) {
			...
			if (isset($gd->is_active)) {
				$gd->is_active = $this->aum->label('is_active', $gd->is_active);
			}
		}
	...
	public function baru()
	{
		...
		$data = new \stdClass();
		if (!$this->aum->validates()) {
			$this->status = 444;
			$this->message = API_ADMIN_ERROR_CODES[$this->status];
			$validation_message = $this->aum->validation_message();
		...
		if ($res) {
			...
			// $update_apikey = $this->aum->update($res, ['apikey' => $token]);
		} else {
			$this->status = 110;
			$this->message = API_ADMIN_ERROR_CODES[$this->status];
		}
	...
	public function hapus($id)
	{
		...
		$res = $this->aum->update($id, array('is_deleted' => 1));
		if ($res) {
			$this->status = 200;
			$this->message = API_ADMIN_ERROR_CODES[$this->status];
		} else {
	...
	{
		$this->load("api_admin/b_user_model", 'aum');
		$d = $this->__init();
		$keyword = $this->input->request('q');
		$ddata = $this->aum->select2($keyword);
		$datares = array();
		$i = 0;
	...
	public function hak_akses()
	{
		...
		$this->load('api_admin/a_pengguna_module_model', 'aumm');
		$a_pengguna_id			= $_POST['a_pengguna_id'];
		$a_modules_identifier	= $_POST['a_modules_identifier'];

		$this->aumm->updateModule(array('tmp_active' => 'N'), $a_pengguna_id);
		...
		foreach ($a_modules_identifier as $ami) {
			...
			$check_ami = $this->aumm->check_access($a_pengguna_id, $ami);
			if ($check_ami == 0) {
				$this->aumm->set($arr);
			} else {
				$this->aumm->updateModule($arr, $a_pengguna_id, $ami);
			}
		}

		$res = $this->aumm->delModule($a_pengguna_id);
	...
	public function pengguna_module()
	{
		...
		$ddata 		= $this->aumm->pengguna_module($id);
		$datares 	= array();
		$i 			= 0;
		foreach ($ddata as $key => $value) {
