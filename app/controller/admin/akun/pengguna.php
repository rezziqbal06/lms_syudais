...
class Pengguna extends JI_Controller {
	...
	private function __accessModules($id="")
	{
		...
		if (!empty($amm))
		{
			...
			foreach ($amm as $am)
			{
				if (empty($id))
				{
					...
					$td .= $this->__accessModules($am->identifier);
					$td .= '</td>';
					if ($n == 2)
					{
						$res   .= '<tr>'. $td .'</tr>';
	...
	public function index(){
		...
		$this->setTitle('Pengguna (Admin) '.$this->config->semevar->site_suffix);
		$data['access']	= $this->__accessModules();
