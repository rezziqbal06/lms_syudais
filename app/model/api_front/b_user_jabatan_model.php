...
class B_User_Jabatan_Model extends \Model\B_User_Jabatan_Concern
{
  ...
  {
    $this->db->select_as("$this->tbl_as.id", 'id', 0);
    $this->db->select_as("$this->tbl_as.a_jabatan_id", 'a_jabatan_id', 0);
  ...
  public function getByUsersId($b_user_id)
  {
    $this->db->select_as("$this->tbl_as.id", 'id', 0);
    $this->db->select_as("GROUP_CONCAT($this->tbl_as.a_jabatan_id)", 'a_jabatan_id', 0);
    $this->db->select_as("$this->tbl_as.b_user_id", 'b_user_id', 0);
    $this->db->select_as("$this->tbl2_as.nama", 'nama', 0);
    $this->db->from($this->tbl, $this->tbl_as);
    $this->db->join($this->tbl2, $this->tbl2_as, "id", $this->tbl_as, "a_jabatan_id");
    $this->db->where_in("$this->tbl_as.b_user_id", $b_user_id)->group_by("$this->tbl_as.b_user_id");
    return $this->db->get($this->tbl);
  }
}
