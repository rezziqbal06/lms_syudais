...
class D_Kehadiran_Model extends SENE_Model
{
	...
	{
		$this->db->select_as("COUNT($this->tbl_as.tgl)", 'nilai', 0);
		$this->db->select_as("$this->tbl_as.e_kegiatan_id", 'e_kegiatan_id', 0);
		...
		$this->db->join($this->tbl3, $this->tbl3_as, 'id', $this->tbl_as, 'e_kegiatan_id');
		$this->db->join($this->tbl4, $this->tbl4_as, 'id', $this->tbl3_as, 'a_program_id');
		$this->db->where_as($this->tbl4_as . '.id', $this->db->esc($id));
		$this->db->order_by("COUNT($this->tbl_as.tgl)", "desc")->group_by("$this->tbl_as.b_user_id")->limit(5);
		return $this->db->get('', 0);
	}
}
