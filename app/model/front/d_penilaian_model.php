...
class D_Penilaian_Model extends \Model\D_Penilaian_Concern
{
  ...
  public function getTopFive($id)
  {
    ...
    $this->db->join($this->tbl3, $this->tbl3_as, 'id', $this->tbl_as, 'b_jadwal_kegiatan_id');
    $this->db->join($this->tbl4, $this->tbl4_as, 'id', $this->tbl3_as, 'a_program_id');
    $this->db->where_as($this->tbl4_as . '.id', $this->db->esc($id));
    $this->db->order_by("AVG($this->tbl_as.nilai)", "desc")->group_by("$this->tbl_as.b_user_id")->limit(5);
