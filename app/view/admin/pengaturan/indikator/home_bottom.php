...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		$().btnSubmit();
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_admin/pengaturan/ruangan/")?>",
...
$("#bhapus").on("click",function(e){
	...
	if(ieid){
		...
		if(c){
			...
			$('.btn-submit').prop('disabled',true);
			$('.icon-submit').addClass('fa-circle-o-notch fa-spin');
			var url = '<?=base_url('api_admin/pengaturan/ruangan/hapus/')?>'+ieid;
...
$("#fl_a_company_id_parent").select2({
	ajax: {
		method: 'post',
		url: '<?=base_url("api_admin/pengaturan/ruangan/get_parent/")?>',
		dataType: 'json',
    delay: 250,
