...
if(jQuery('#drTable').length>0){
	...
	.on('preXhr.dt', function ( e, settings, data ){
		$().btnSubmit();
			...
			"bProcessing"		: true,
			"bServerSide"		: true,
			"sAjaxSource"		: "<?=base_url("api_admin/pengaturan/jabatan/")?>",
			...
			"fnServerData"	: function (sSource, aoData, fnCallback, oSettings) {
				oSettings.jqXHR = $.ajax({
					...
					$('#drTable > tbody').on('click', 'tr', function (e) {
						...
						ieid = id;
						$.get('<?=base_url("api_admin/pengaturan/jabatan/detail/")?>'+ieid).done(function(dt){
							if(dt.data){
...
$("#ftambah").on("submit",function(e){
	...
	var url = '<?= base_url("api_admin/pengaturan/jabatan/baru/")?>';
...
$("#fedit").on("submit",function(e){
	...
	var fd = new FormData($(this)[0]);
	var url = '<?=base_url("api_admin/pengaturan/jabatan/edit/")?>';
...
$("#bhapus").on("click",function(e){
	...
	if(ieid){
		...
		if(c){
			...
			$('.btn-submit').prop('disabled',true);
			$('.icon-submit').addClass('fa-circle-o-notch fa-spin');
			var url = '<?=base_url('api_admin/pengaturan/jabatan/hapus/')?>'+ieid;
...
$("#fl_a_company_id_parent").select2({
	ajax: {
		method: 'post',
		url: '<?=base_url("api_admin/pengaturan/jabatan/get_parent/")?>',
		dataType: 'json',
    delay: 250,
